...
class RegisterCommandTest {
  ...
  void testExecuteWrongNumberOfArgs() {
    this.registerCommand.execute(mock(MinecraftServer.class), sender, new String[] {"test", "test2", player.getEmail(), player.getPassword()});
  ...
  void testExecute() {
    this.registrator = new Registrator(this.dataSourceStrategy);
    this.registerCommand.execute(mock(MinecraftServer.class), sender, new String[] {player.getPassword(), player.getPassword()});
  ...
  void testExecuteWithEmailRequired() {
    ...
    this.registrator = new Registrator(this.dataSourceStrategy);
    this.registerCommand.execute(mock(MinecraftServer.class), sender, new String[] {player.getEmail(), player.getPassword(), player.getPassword()});
  ...
  void testExecuteInvalidEmail() {
    this.registerCommand = new RegisterCommand(this.handler, this.dataSourceStrategy, true);
    this.registrator = new Registrator(this.dataSourceStrategy);
    this.registerCommand.execute(mock(MinecraftServer.class), sender, new String[] {"not_an_email", player.getPassword(), player.getPassword()});
    assertFalse(this.handler.isLogged(this.sender));
  }

  @Test
  void testExecuteWrongPasswordConfirmation() {
    this.registerCommand.execute(mock(MinecraftServer.class), sender, new String[] { player.getPassword(), player.getPassword() + "error"});
    assertFalse(this.handler.isLogged(this.sender));
  }
  ...
  @Test
  void testCompareTo() {
    assertEquals(0, this.registerCommand.compareTo(new RegisterCommand(this.handler, new FileDataSourceStrategy())));
  }
}
